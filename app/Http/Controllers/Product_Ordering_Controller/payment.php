...
class payment extends Controller
{
    ...
    public function payumoneyResponse(Request $request)
    {
        ...
        if($status == "success" && $unmappedstatus == 'captured')
        {
            ...
            $emailbody ='<p>Your Payment '.$amount.' towards Order '.$O_id. 'is Successfully Paid. Your Order is Confirmed. Estimated Delivery 3-5 Working days</p>
            <br>
            <h4>Order Details: </h4>
            <p> Order No:'.$O_id.$Order_Details.'</p>
            ...
            ($loginid, $name, $O_id)
            {
                ...
                ('Your Superfoods.in Order Payment '.$O_id.' is Successfully Paid');
            ...
            {
                $message->to($loginid, $name)->subject
                ('Your Superfoods.in Order '.$O_id.' Payment is Failed');
                $message->from('vasanthaws28@gmail.com','SuperFoods');
            });
            Session::forget('cart');
            Session::forget('discount');
